...
class ControlTower(Node):
    ...
    def __init__(self):
        ...
        self.ff = [1.574251224, 0.628, -0.855, -0.185900708, 0.262, -0.1, 0.0, 0.0]
        # self.f1 = [1.6057, 0.8727, -0.5411, 0.1047, -0.3491, -0.0524 , 0.0, 0.0]
        # self.f2 = [1.6057, 0.8727, -0.5236, 0.1571, -0.2094, -0.0873, 0.0, 0.0]
        # self.f3 = [1.6406, 0.9425, -0.3665, 0.1396, -0.4538, -0.0698, 0.0,0.0]
        # self.f4 = [1.6581, 1.0996, -0.1920, 0.1222, -0.7505, -0.0349, 0.0, 0.0]
    ...
    def send_pose_wm(self, pose: PoseStamped):
        ...
        goal_msg.lock_rp_and_cartesian = True  # 'lock_roll_pitch' -> 'lock_rp_and_cartesian'
        goal_msg.roll_tol_rad = 3.14               # 'tol_roll'/'tol_pitch' -> 'rp_tol_rad'
        goal_msg.pitch_tol_rad = 0.1
        goal_msg.yaw_tol_rad = 3.14 
    ...
    def send_pose_ba(self, pose: PoseStamped):
        ...
        goal_msg.lock_rp_and_cartesian = True  # 'lock_roll_pitch' -> 'lock_rp_and_cartesian'
        goal_msg.roll_tol_rad = 1.0               # 'tol_roll'/'tol_pitch' -> 'rp_tol_rad'
        goal_msg.pitch_tol_rad = 1.0
        goal_msg.yaw_tol_rad = 3.14              # 'tol_yaw' -> 'yaw_tol_rad'
        
        # eef_step, jump_threshold도 필요 시 설정해줄 수 있습니다.
        # 설정하지 않으면 C++ 서버의 기본값을 사용합니다.
        goal_msg.eef_step = 0.01
        goal_msg.jump_threshold = 0.0
        # --- 수정 끝 ---
    ...
    def target_get_wm(self, mode="failure", first=True): ### basket, wm, mid
        ...
        if result.success:
            ...
            while not self.send_pose_ba(pose_stamped):
            ...
            while not self.goal(self.f, 'keep'):
    ...
    def execute_sequence(self):
        ...
        while rclpy.ok():
            ...
            while True:
                self.get_logger().info("Step 2: 잡기 시도 중...")
                self.target_get_ba(mode="basket")  # 감지 및 잡기 시도
                #### 잡고 다시 ba_init으로 옴
                self.get_logger().info("🔍 Z-축 정렬 감지")
