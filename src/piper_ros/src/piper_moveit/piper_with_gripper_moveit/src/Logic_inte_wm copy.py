...
class ControlTower(Node):
    ...
    def __init__(self):
        ...
        self.ff = [1.588, 1.693, -2.007, 0.000, 0.506, 0.000, 0.0, 0.0]
        self.last = [-1.571, 1.117, -1.484, 0.000, 0.000, 0.000, 0.0, 0.0]
    ...
    def send_pose_wm(self, pose: PoseStamped):
        ...
        goal_msg.pitch_tol_rad = 3.14/6           # 'tol_roll'/'tol_pitch' -> 'rp_tol_rad'
    ...
    def target_get(self, mode="wm"): ### basket, wm, mid
        ...
        if result.success:
            ...
            # success = self.send_pose_ba(pose_stamped)
            # self.close_gripper()

            # while not self.goal(self.ff, 'keep'):
            #     self.get_logger().warn("  >> 실패 → 재시도 중")

            while not self.send_pose_wm(pose_stamped):
    ...
    def execute_sequence(self):
        ...
        while rclpy.ok():
            ...
            while True:
                ...
                self.target_get(mode="wm")  # 감지 및 잡기 시도
            ...
            # while not self.goal(self.last, 'keep'):
            #     self.get_logger().warn("  >> 실패 → 재시도 중")
