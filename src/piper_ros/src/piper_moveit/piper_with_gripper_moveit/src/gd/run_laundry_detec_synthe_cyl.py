...
def create_cylinder_shell(center_xy, radius, z_min, z_max, radial_res=200, height_res=150, color=[1, 0, 0]):
    cx, cy = center_xy
...
def run(mode:str, output_json=False):
    cfg = CFG[mode]
    ...
    try:
        while True:
            ...
            if 1: #key == ord('d'):
                ...
                if mode == "wm" and filtered:
                    ...
                    # centers, clusters에서 날개 영역 제거
                    def is_in_rotated_filtered(c):
                        for f in rotated_filtered:
                            if np.linalg.norm(c - f.mean(axis=0)) < 0.05:
                                return True
                        return False
                    filtered_centers, filtered_clusters = [], []
                    for c, cl in zip(centers, clusters):
                        if not is_in_rotated_filtered(c):
                            filtered_centers.append(c)
                            filtered_clusters.append(cl)
                    centers, clusters = filtered_centers, filtered_clusters
        ...
        print("[ERROR] 런타임 예외 발생:", e)
        traceback.print_exc()  # 👈 어떤 에러인지 전체 스택 출력
        if output_json:
...
if __name__ == "__main__":
    ...
    run(args.mode, output_json=args.json)  # ✅ 수정된 부분
