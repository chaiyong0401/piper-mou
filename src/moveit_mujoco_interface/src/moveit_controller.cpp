...
ArmController::ArmController(std::string node_name):Node(node_name)
{
    ...
    joint_command_pub = this->create_publisher<sensor_msgs::msg::JointState>("piper/joint_set", 10);
    joint_command_msg.name.resize(6);
    joint_command_msg.position.resize(6);
    joint_command_msg.velocity.resize(6);
    joint_command_msg.header.frame_id="link0";
    
    for(int i=0;i<6;i++){
        joint_command_msg.name[i] = "joint" +std::to_string(i+1);
    }
...
GripperController::GripperController(std::string node_name):Node(node_name)
{
    ...
    joint_command_msg.header.frame_id="link0";
    joint_command_msg.name[0]="joint7";
    joint_command_msg.name[1]="joint8";
}
